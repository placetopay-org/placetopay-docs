...
### Paso 1: Crear una sesión de Checkout (Check-in)
...
Envía una `RedirectRequest` al [endpoint de creación de sesión](/checkout/api/reference/session#create-a-session):

* `type` debe ser `"checkin"`
* No se permiten pagos mixtos ni dispersos
* La moneda se mantiene fija durante todo el flujo

```json {{ 'title': 'REQUEST RedirectRequest - Checkin' }}
{
  ...,
  "type": "checkin",
  "payment": {
...
```json {{ 'title': 'RESPONSE RedirectRequest - Checkin' }}
{
  "status": {
    "status": "OK",
    "reason": "PC",
...
Redirige al usuario al `processUrl` para completar este paso. Una vez finalizado, usa [Consultar sessión](/checkout/api/reference/session#query-a-session) (getRequestInformation) para confirmar el estado:
...
Para actualizar el monto reservado, envía una `TransactionActionRequest` al endpoint de [Acciones sobre transacciones](/checkout/api/reference/payment#transaction-actions):

* `action` debe ser `"reauthorization"`
* Puedes realizar múltiples reautorizaciones
* La moneda y la referencia se heredan del Check-in

```json {{ 'title': 'REQUEST TransactionActionRequest - Reautorización' }}
{
  "auth": { ... },
...
```json {{ 'title': 'RESPONSE TransactionActionRequest - Reautorización' }}
{
  "status": {
    "status": "APPROVED",
    "reason": "00",
...
Para capturar o liberar los fondos, envía otra `TransactionActionRequest` al [Acciones sobre transacciones](/checkout/api/reference/payment#transaction-actions):

* `action` debe ser `"checkout"`
* Establece `total` en `0` para liberar los fondos
* La moneda y la referencia deben coincidir con el Check-in
