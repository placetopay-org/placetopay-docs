...
Sesión es el término que usamos para referirnos al proceso de pago con el que interactúan tus usuarios en Checkout. {{ className: "leading" }}

Para crear una sesión debes consumir nuestra API según tus necesidades, conoce los detalles del API en [API - Crear Sesión](/checkout/api/reference/session#create-a-session).
...
Cuando se hace una sesión de pagos parciales, aparecen nuevos escenarios que se deben controlar:
...
- La sesión finaliza con el estado `PARTIAL_EXPIRED`, ocurre cuando en la sesión se hizo al menos una transacción `APPROVED`, pero no se completó el total del monto solicitado y la sesión expiró.
...
Una sesión de pago recurrente es una sesión en la cual el usuario completa el proceso con una tarjeta de crédito, y a partir de ese momento periódicamente se le realizan cobros automáticos al medio de pago. Por ejemplo, en el pago de una mensualidad o suscripción a un servicio.
...
- El atributo `nextPayment` no es obligatorio, en caso de que no se envíe, se calcula dependiendo del atributo `interval` y `periodicity`, en caso de que se declare debe ser una fecha futura a la fecha actual.
...
Para hacer uso de esta funcionalidad es importante conocer el identificador del convenio (`agreement`) a donde va dirigido el pago. Si no se agrega ningún valor de agreement, se tomará por defecto el medio de pago configurado en el sitio de la sesión, también es necesario indicar el tipo de convenio (`agreementType`), en este caso MERCHANT o AIRLINE.
...
Para el envío de impuestos, es importante que se agreguen en la estructura [amount](/payments/microsite-customized/data-structures#amount) de cada una de las dispersiones
...
Redirige al usuario al `processUrl` para completar este paso. Una vez finalizado, usa [API Web Checkout -> Consultar sesión](/checkout/api/reference/session#query-a-session) (getRequestInformation) para confirmar el estado:
...
* La moneda debe coincidir con el Check-in
...
La suscripción permite almacenar de forma segura (tokenizando) el medio de pago de un usuario, para que luego pueda efectuar cobros relacionados a este.
...
<Note>Si el usuario no activa la opción de *guardar el medio de pago*, este **no será almacenado** y la sesión de checkout se completará como un pago único, sin generar una suscripción.</Note>
...
```json {{ 'title': 'Respuesta - QuerySessionRequest' }}
{
  "requestId": 1,
  "status": {...},
  "request": {
