...
Durante el proceso de pago, se mostrará un mensaje similar a este:  
**¿Deseas que PlacetoPay y [Nombre del Comercio] guarden tu información para futuros pagos?**   

Este mensaje permite al usuario autorizar el almacenamiento seguro de su medio de pago (tokenización).
...
<Note>
El usuario deberá activar la opción de **guardar medio de pago** para autorizar la tokenización de su tarjeta.   
Si esta opción no es activada, el medio de pago no será guardado, incluso si subscribe fue enviado como **true**.
</Note>

### Flujo completo

1. Enviar **`subscribe: true`** en `CreateSessionRequest - Payment with Subscription`.
2. El usuario activa la opción **Guardar medio de pago** durante el proceso de pago.
3. Una vez completado el pago, se genera un token vinculado al medio de pago autorizado.
4. Consulta la información de la sesión para obtener el token utilizando el endpoint **[Consultar una sesión](/checkout/api/reference/session#query-a-session)**.
5. Usa el token obtenido para realizar cobros posteriores mediante el endpoint **[Cobro usando token](/checkout/api/reference/token#charge-using-token)**.
