...
Para realizar un reembolso debes conocer la referencia interna de la transacción a reembolsar `payment.internalReference`. Este dato se puede obtener al [consultar una sesión](/checkout/api/reference/session#query-a-session).

## ¿Cómo realizar un reembolso? {{ id: 'how-to-make-a-refund' }}

Para realizar un reembolso se debe hacer una petición al endpoint [Reembolsar Transacción](/checkout/api/reference/payment/#transaction-actions). De acuerdo a la necesidad es posible hacer reembolsos totales y reembolsos parciales.
...
Si la transacción no ha sido conciliada en la red bancaria, no se pueden realizar reembolsos parciales, se debe realizar un reembolso total.
...
Para verificar que una transacción fue reembolsada total o parcialmente, se debe [consultar la sesión](/checkout/api/reference/session#query-a-session) de la transacción original donde encontrarás en la respuesta la propiedad `payment.refunded` con valor `true`.
