...
Sigue estos pasos para completar tu integración de forma exitosa.
...
La url referenciada en el script puede cambiar de acuerdo al país y proveedor comercial con el que te estés integrando.
...
Redireccionar es el comportamiento predeterminado; sin embargo, puedes cambiar el comportamiento definiendo la propiedad `backupTarget` conoce más en [Alternativas de acceso a la URL de procesamiento](#alternative-access-process-url).
</Note>
...
También es posible suscribirse a más de un evento del mismo tipo, solo debes registrar cada uno de forma separada en `P.on()`.

### Alternativas de acceso a la URL de procesamiento {{ id: 'alternative-access-process-url' }}

Si el lightbox no puede abrirse de manera normal las alternativas de acceso a la URL de procesamiento dependerán de la propiedad `backupTarget` que se le pase a la configuración del lightbox, las alternativas son:
