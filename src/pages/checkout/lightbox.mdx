...
<Note>
Ten en cuenta que si el ambiente no es propicio para que el lightbox se abra de manera normal, se implementará una forma alternativa de acceder a la URL de procesamiento para que el usuario pueda continuar con el proceso.
</Note>
...
| `newtab` | Apertura de nueva pestaña con la URL de procesamiento |
...
| `options.backupTarget` | `string` | Define el comportamiento alternativo cuando el lightbox no puede abrirse normalmente. <br/> Por Defecto: `self` <br/> Valores posibles: `self`, `popup`, `newtab` <br/> `self`: Redirección a la URL de procesamiento <br/> `popup`: Apertura de un popup con la URL <br/> `newtab`: Apertura en nueva pestaña |
