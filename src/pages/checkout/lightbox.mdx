...
<Note>
Ten en cuenta que si el lightbox no encuentra un ambiente propicio para su funcionamiento, se ejecutará una redirección a la URL de procesamiento. Por lo tanto, es importante asegurarse de que puedas manejar la respuesta en el backend de tu aplicación a través del proceso de `returnUrl` como de costumbre.
</Note>
