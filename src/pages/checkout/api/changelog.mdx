...
- Se ajusto la validación de las propiedades `payer.name`, `payer.surname`, `buyer.name` y `buyer.surname` como un string de máximo 60 caracteres
- Se agrega `metadata.athMovilConfirmationTimeOut` a `CreateSessionRequest` para configurar entre `1` y `60` minutos el tiempo de confirmación de pagos con ATH Móvil. También admite `null`. Ver [Metadata](/checkout/metadata) y [ATH Móvil](/payments/external-redirects/ath-movil#confirmation-timeout).
