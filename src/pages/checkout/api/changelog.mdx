...
### Nuevo:
- Se agreaga un limite de reintentos en los procesos de pago y tokenización dentro de una sesión. Este límite se aplica a los intentos fallidos (transacciones o tokenizaciones con estado `REJECTED`) y se puede configurar por sitio. El valor predeterminado es `10` intentos fallidos. Si se supera este límite, la sesión se marca con el estado `REJECTED` y no se permiten más procesos en esa sesión. [Límite de intentos](/checkout/attempts-limit)
