...
Agrega validación para el número máximo de dispersiones permitidas en los [pagos con dispersión](/checkout/create-session#dispersion-payment) en los endpoints `api/session` y `api/collect`. El límite predeterminado es 3 dispersiones. 
Ahora, Si se supera este límite, se responde con un error `status.status` `FAILED` y `status.reason` `max_dispersion_error`.
