...
- Se agrega soporte para registrar instrumentos y tokenizar cuando el sitio tiene activas billeteras externas (`Google Pay` y `Click to Pay Visa`).
- Se agrega soporte para descuentos `Zunify`.

### Cambió:

- Se refuerza la validación de las propiedades `name`, `surname`, `email`, `mobile`, `document` y `documentType` del pagador.
- Al consultar una sesión que pertenece a otro sitio, se responde con el mensaje genérico de sesión inexistente.
- Se agrega soporte para la nueva estructura de créditos en el endpoint [Solicitud de Información](/checkout/api/reference/gateway/information) `/api/gateway/information`.

## `2.32.0` 2026-07-02

### Nuevo:

- Se agrega la propiedad `metadata.requiresConfirmation` para habilitar el flujo de confirmación de transacciones. Al establecerla en `true`, la transacción se crea en estado `PENDING_CONFIRMATION` y el comercio puede confirmarla o cancelarla antes de la autorización final. Requiere tener habilitada la configuración `allowConfirmationFlow` en el sitio. Ver más en [Flujo de confirmación](/checkout/confirmation-flow) y [metadata](/checkout/metadata).

## `2.31.0` 2026-02-20

### Cambió:

- Se aplican límites y recortes a los datos de la petición en los endpoints `/api/session` y `/api/collect` antes de crear la sesión:
  - `payment.items` admite un máximo de `50` elementos; los elementos adicionales se descartan.
  - Los campos `sku`, `name` y `qty` de cada elemento se recortan a un máximo de `30`, `120` y `30` caracteres, respectivamente.
  - Las direcciones de `payer`, `buyer` y `payment.shipping` recortan los campos `city`, `postalCode` y `street` a un máximo de `30`, `10` y `128` caracteres.

## `2.30.0` 2024-10-28

### Nuevo:

- Agrega soporte para uso del algoritmo Sha256 en el firmado de mensajes de notificación enviados al comercio. 
  - Ver más en [Notificaciones](/checkout/notification)
  - Para migrarse a sha256, sigue la guía de [Migración a sha256](/checkout/migration-sha256).

## `2.29.0` 2024-09-08

### Nuevo:

- Endpoint [Consultar tokenización](/checkout/api/reference/gateway/token) `/api/gateway/token`. Servicio que permite buscar una tokenización mediante el valor del token o subtoken.
