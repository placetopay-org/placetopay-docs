...
Se identifica como token, a la llave generada por un proceso de suscripción permitiendo generar cobros sin interacción del usuario. {{ className: 'lead' }}
 ...
 <Note type="error"> La autenticación 3D Secure (3DS) aplica únicamente al proceso de tokenización inicial que consiste en la validación de la tarjeta. Las transacciones posteriores realizadas con el token no están autenticadas por 3DS; por lo tanto, ante reclamaciones o contracargos, la responsabilidad recae en el comercio. Considere este escenario dentro de su gestión de riesgo. </Note>
