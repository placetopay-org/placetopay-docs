...
export const description =
    'Aprende a controlar la notificación (Webhook) que Checkout envía a tu sitio cuando una sesión es finalizada.';
...
## Estructura de la notificación `MerchantNotificationRequest` {{ id: 'notification-structure', tag: 'POST', label: '/{merchant-notification-url}' }}
...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="MerchantNotificationRequest" tag="POST" label="/merchant-api/notification-url">
...
<Note>
  En versiones más recientes de nuestra API, se ha marcado como obsoleta la generación de firmas utilizando el algoritmo SHA-1. Asegúrate de actualizar tu integración para utilizar SHA-256, que es el método recomendado y seguro en la actualidad. Ver más en [Proceso de migración de SHA-1 a SHA-256](/checkout/migration-sha256)
</Note>
...
Con esto obtendrás la firma generada por tu sistema, llamaremos a esta `generatedSignature`.
...
Los pagos recurrentes ocurren después de que la sesión inicial se completó, y generan notificaciones similares a las de una sesión normal, con algunas excepciones:
...
Si actualmente estás utilizando SHA-1 para la generación de firmas en tus notificaciones, es altamente recomendable que migres a SHA-256 para mejorar la seguridad de tu integración. Dirígete a nuestra guía de [Migración de notificaciones a SHA-256](/checkout/migration-sha256/) para obtener instrucciones detalladas sobre cómo realizar esta transición de manera efectiva.
