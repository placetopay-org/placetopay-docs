...
### Consideraciones de Seguridad {{ id: 'security-considerations' }}

#### Prohibición de Uso en Navegador
...
fetch('https://webcheckout-api/payment', { 
    body: JSON.stringify({ apiKey: "TU_CLAVE" }) // ¡Clave expuesta!
});

// ✅ Correcto (servidor)
app.post('/procesar-pago', async (req, res) => {
    const response = await axios.post('https://webcheckout-api/payment', {
        apiKey: process.env.SECURE_API_KEY // Clave protegida
    });
    // [...] 
});
```
