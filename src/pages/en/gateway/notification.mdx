export const description = 'Gateway HTTP notifications: reception, SHA256 signature validation, and transaction status update.'
...
The signature is generated with the following formula: `SHA256(internalReference + status.status + secretKey)`
...
To ensure the authenticity of the notification, the session webhook includes a signature in the `X-Signature` header. You can validate this signature using an SHA256 hash generated with the values of the session ID, session status, and the transaction key (`tranKey`) of your site.
