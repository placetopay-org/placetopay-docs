...
export const description = '3DS Gateway reference for initial inquiry and authentication validation.'

<ApiDownloads />

## 3DS Lookup {{ id: 'mpi-lookup-request', tag: 'POST', label: '/gateway/mpi/lookup' }}

Performs the initial validation process of [3DS Server](/en/three-d-s-server/api/integration/session), returning the data so that the user can continue with the authentication process.
