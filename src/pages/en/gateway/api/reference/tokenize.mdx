...
export const title = 'Tokenization (tokenize)'
export const description = 'Tokenization of credit cards and bank accounts in Gateway, with token consultation and invalidation.'
...
<Row>
    <Col>
        ...
        > Prior to consuming this service, the information must be consulted to know whether or not it is necessary to generate an OTP for the client and, if necessary, request the token from the person and send it during consumption.
         ...
         <Note type="error">It is important to note that payments made with registered tokens (cards) do not include additional 3DS security validation, meaning that these transactions will not be authenticated under this protocol. In this regard, in the event of a claim or chargeback, the responsibility lies with the merchant, as there is no coverage associated with 3DS authentication. We therefore recommend evaluating this aspect as part of your risk management strategy. If you need to enable it for future transactions, please contact your assigned sales representative to discuss your case. </Note>
