...
A valid value sent by the merchant takes precedence over the site configuration. If it is omitted, Placetopay uses the site’s `initiatorIndicator` setting; if no site value is configured, Placetopay determines it automatically. See [site configuration and resolution order](/en/gateway/initiator-indicator).
