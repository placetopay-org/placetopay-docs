export const description = 'Idempotency in Gateway with idempotenceKey to prevent duplicate payment transactions.'
...
> This service is not intended and should not be taken as a change to the integration flow established in [integration flow](/en/gateway/transaction-flow) particularly point 4 thereof. This is an additional control that can be implemented