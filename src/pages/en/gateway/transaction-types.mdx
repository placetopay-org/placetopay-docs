...
export const description = 'Types of transactions in Gateway: authorizations, recurring payments, preauthorizations, and payment dispersion.'
...
## Time limits for keeping a pre-authorization active

The gateway applies a maximum validity period for pre-authorizations, according to the configuration defined for each provider.

If a pre-authorization exceeds this period without being confirmed/captured, the gateway will automatically initiate the reversal (`reverse`) process to release the held funds.

| Provider   | Default time limit |
|------------|-------------------:|
| Transerver |            30 days |
| Paystudio  |            30 days |
| Ebus       |            30 days |
...
<Note type='warning'>
    These time limits may vary depending on the network/provider rules.
</Note>

### Recommendation for integrators

To avoid unexpected automatic reversals:
- confirm/capture the pre-authorization within the allowed time, or
- implement monitoring/alerts for pre-authorizations that are close to expiring.
