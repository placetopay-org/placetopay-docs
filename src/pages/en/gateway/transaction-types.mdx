...
This step reserves a specific amount on the customer’s card. [API Gateway -> Transaction processing](/en/gateway/api/reference/transaction#process-request)

* `action` must be `checkin`.
* `payment.reference` and `payment.amount` are required.
* The response will include a `preAuthorization` object with values needed for subsequent steps.
...
This optional step increases the held amount before capture. [API Gateway -> Operations on a transaction](/en/gateway/api/reference/transaction#transaction-request)

* You must reuse `internalReference` and `authorization` from the `preAuthorization` object returned in the **Check-in** step.
...
This step captures or releases the held amount. [API Gateway -> Operations on a transaction](/en/gateway/api/reference/transaction#transaction-request)

* `action` must be `checkout`.
* If `amount.total` is `0`, the hold is released.
* Must include `internalReference` and `authorization` from the last step.
