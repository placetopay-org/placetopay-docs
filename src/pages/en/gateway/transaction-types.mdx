...
This step reserves a specific amount on the customer’s card.

This is done by sending the `RequestBody` to the [API Gateway -> Transaction processing](/en/gateway/api/reference/transaction#process-request).
...
```json {{ 'title': 'REQUEST - RequestBody -> checkin' }}
{
    "auth": {{auth}},
    "action": "checkin",
    "payment": {
...
This optional step updates the held amount before capture. 

This is done by sending the `RequestBody` to the [API Gateway -> Operations on a transaction](/en/gateway/api/reference/transaction#transaction-request).
...
```json {{ 'title': 'REQUEST - RequestBody -> reauthorization' }}
{
    "auth": {{auth}},
...
```json {{ 'title': 'REQUEST - RequestBody -> checkout' }}
{
    "auth": {{auth}},
    "internalReference": 30,
    "authorization": "235263",
