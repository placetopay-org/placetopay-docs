...
- Adjusted validation for `payer.name`, `payer.surname`, `buyer.name` and `buyer.surname` properties to a string of maximum 60 characters.
- Adds `metadata.athMovilConfirmationTimeOut` to `CreateSessionRequest` to configure an ATH Móvil payment confirmation timeout from `1` through `60` minutes. It also accepts `null`. See [Metadata](/en/checkout/metadata) and [ATH Móvil](/en/payments/external-redirects/ath-movil#confirmation-timeout).

## `2.32.0` 2026-07-02

### Added:

- Adds the `metadata.requiresConfirmation` property to enable the transaction confirmation flow. When set to `true`, the transaction is created in `PENDING_CONFIRMATION` status and the merchant can confirm or cancel it before the final authorization. It requires the `allowConfirmationFlow` setting to be enabled on the site. See more in [Confirmation flow](/en/checkout/confirmation-flow) and [metadata](/en/checkout/metadata).

## `2.31.0` 2026-02-20

### Changed:

- Applies limits and shortening to request data in the `/api/session` and `/api/collect` endpoints before creating the session:
  - `payment.items` accepts a maximum of `50` items; additional items are discarded.
  - The `sku`, `name` and `qty` fields of each item are shortened to a maximum of `30`, `120` and `30` characters, respectively.
  - The `payer`, `buyer` and `payment.shipping` addresses shorten the `city`, `postalCode` and `street` fields to a maximum of `30`, `10` and `128` characters.

## `2.30.0` 2024-10-28

### New:

- Adds support for using the Sha256 algorithm in the signing of notification messages sent to the merchant.
  - See more in [Notifications](/en/checkout/notification)
  - To migrate to sha256, follow the guide [Migrating to sha256](/en/checkout/migration-sha256).
...
- Control for `*.fields` properties in the session creation request `/api/session`. [Additional fields](/en/checkout/additional-fields)
