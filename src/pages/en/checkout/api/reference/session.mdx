...
```
```bash {{ title: 'Create a taxed session' }}
curl -X "POST" https://checkout-test.placetopay.com/api/session \
  -H "Content-Type: application/json" \
  -d '{
    "locale": "es_CO",
...
```
```bash {{ title: 'Create dispersion session' }}
curl -X "POST" https://checkout-test.placetopay.com/api/session \
  -H "Content-Type: application/json" \
  -d '{
    "locale": "es_CO",
...
```
```bash {{ title: 'Create subscription session' }}
curl -X "POST" https://checkout-test.placetopay.com/api/session \
  -H "Content-Type: application/json" \
  -d '{
    "locale": "es_CO",
...
```
```bash {{ title: 'Create recurring session' }}
curl -X "POST" https://checkout-test.placetopay.com/api/session \
  -H "Content-Type: application/json" \
  -d '{
    "locale": "es_CO",
    ...
    "payment": {
      "reference": "331122",
      "description": "Payment with recurring",
      "amount": {
        "currency": "USD",
        "total": 100
      },
...
```
```bash {{ title: 'Create modifiers session' }}
curl -X "POST" https://checkout-test.placetopay.com/api/session \
  -H "Content-Type: application/json" \
  -d '{
    "locale": "es_CO",
...
curl -X "POST" https://checkout-test.placetopay.com/api/session \
  ...
  -d '{
    ...
    "payment": {
      "reference": "12345",
      "description": "Test of payment and subscription",
      "amount": {
        "currency": "COP",
        "total": 2000
      },
