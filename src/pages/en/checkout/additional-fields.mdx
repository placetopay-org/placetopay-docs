...
<Note>
Data with property `display: both, receipt, payment, receipt, approved`, will be displayed in some interface flow.
</Note>

## Conditions

- up to 50 additional fields. These can be sent in `fields`, `payment.fields`, or `subscription.fields`. Fields will first be validated in `fields`, with any remaining allocated to `payment.fields or subscription.fields`.
- Up to 255 characters in the `value` property.
- Up to 50 characters in the `keywork` property.
