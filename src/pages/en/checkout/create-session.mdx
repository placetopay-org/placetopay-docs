...
During the payment process, a message similar to the following will be displayed:  
**Would you like PlacetoPay and [Merchant Name] to save your information for future payments?**

This message allows the user to authorize the secure storage (tokenization) of their payment method.
...
<Note>
The user must enable the **save payment method** option to authorize the tokenization of their card.  
If this option is not enabled, the payment method will not be saved, even if `subscribe` was set to **true**.
</Note>

### Full Flow

1. Send **`subscribe: true`** in `CreateSessionRequest - Payment with Subscription`.
2. The user enables the **Save payment method** option during the payment process.
3. Once the payment is completed, a token linked to the authorized payment method is generated.
4. Retrieve the session details to obtain the token using the **[Query a session](/checkout/api/reference/session#query-a-session)** endpoint.
5. Use the obtained token to perform future charges through the **[Charge using token](/checkout/api/reference/token#charge-using-token)** endpoint.
