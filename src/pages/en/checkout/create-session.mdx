...
- The session ends with the `APPROVED_PARTIAL` status, it occurs when at least one `APPROVED` transaction was made in the session, but the total requested amount was not completed.
...
- The total sum of the dispersion amounts must be equal to the total payment amount.
- All dispersion currencies must be equal to the payment currency

For the sending of taxes, it is important that it is in the [amount](/en/payments/microsite-customized/data-structures#amount) structure of each of the dispersions
...
Send a `RedirectRequest` to [API Web Checkout -> Session create](/en/checkout/api/reference/session#create-a-session):
...
> The `RedirectInformation.payment` is an array of attempts about payment, if the `RedirectInformation.status` is `APPROVED`, identify the successful attempt.
...
<Note>
    ...
    The protection consists of `payment.reference`, `payment.amount.total` and `payment.amount.currency`
...
<Note>If the user does not enable the option to *save the payment method*, it **will not be stored** and the checkout session will be completed as a one-time payment, without generating a subscription.</Note>
...
To do so, send a request to the [Query a session](/en/checkout/api/reference/session#query-a-session) endpoint using the corresponding `requestId`.  The token will be available in the `subscription.instrument` field of the response, if the process was successful.

```json {{ 'title': 'Response - Query SessionRequest' }}
{
  "requestId": 1,
  "status": {...},
  "request": {
