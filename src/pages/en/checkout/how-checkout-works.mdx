...
Para aceptar un pago a través de Checkout, es necesario crear una sesión de pago (**Checkout Sesssion**) utilizando el método [API - Crear sesión (CreateRequest)](/en/checkout/api-reference/session#crear-una-sesion).  
Al llamar a este servicio, se obtendrá la URL de proceso (`processURL`) y el identificador de solicitud (`requestId`).  
Puedes ver más en el apartado [Crear Sesión](/en/checkout/api-reference/session#crear-una-sesion)
...
Esto se puede lograr utilizando el método [API - Consultar sesión (getRequestInformation)](/en/checkout/api-reference/session#consultar-una-sesion).
...
- [Crear Sesión](/en/checkout/create-session)
- [Plugins y librerias](/en/checkout/plugins)
