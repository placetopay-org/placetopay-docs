...
- [1] The concept of "Captured Transaction" refers to the "Transaction Confirmation Workflow", to integrate with this flow, go to [Transaction confirmation](/en/checkout/confirmation-flow).
...
A **checkout session** is the flow or process that allows the user to generate a final result. This result can be one of the following entities:

- **Payment transaction** (`payment`): A single or multiple payment transaction depending on the configured session type.
- **Tokenization** (`subscription`): A subscription that stores the user's payment method for future charges.
- **Autopay** (`autopay`): An autopay that will enable automatic recurring charges.

Once the checkout session has generated its final result, the session process **terminates definitively**. From this point forward, any modifications or additional operations must be performed on the **resulting entities** (payment, subscription, or autopay), not on the checkout session.

<Note>
Modifications made to the resulting entities **will not be reflected** in checkout session queries. The session represents only the moment of capture and creation of the result, not its subsequent lifecycle.
</Note>

### Post-session operations examples

The following operations are performed on the resulting entities and will **not** affect the session state:

- **Transaction reversal**: If a payment transaction (`payment`) is reversed later, this change will not be reflected when querying the session. The session will continue to show the `APPROVED` status it had at the time of its finalization.

- **Payment refund**: Refunds (full or partial) are performed on the payment transaction, not on the session. When querying the session, you will not see information about refunds made.

- **Subscription cancellation**: If a subscription (`subscription`) is canceled after being successfully created, the checkout session that originated it will continue to show the `APPROVED` status and will not reflect the cancellation.

- **Autopay cancellation**: Similar to subscriptions, if an autopay (`autopay`) is deactivated or canceled, this change will not appear in the original session queries.

To track or modify these entities, the integrating merchant must use the APIs corresponding to each entity type:
...
- [Autopay API](/en/autopay/api) for autopays
...
- [Transaction confirmation](/en/checkout/confirmation-flow)
