...
<Note>
Note that if the lightbox cannot find a suitable environment to operate normally, an alternative method of accessing the processing URL will be implemented so that the user can continue with the process.
</Note>
...
| `newtab` | Open in new tab with the processing URL |
...
| `options.backupTarget` | `string` | Defines the alternative behavior when the lightbox cannot be opened normally. <br/> Default: `self` <br/> Possible values: `self`, `popup`, `newtab` <br/> `self`: Redirect to the processing URL <br/> `popup`: Open a popup with the URL <br/> `newtab`: Open in new tab |
