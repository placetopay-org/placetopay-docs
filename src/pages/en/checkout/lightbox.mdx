...
export const description =
  'Learn about Lightbox, a way to integrate Checkout into your site or app without having to redirect your users to another page.'
...
A lightbox containing the checkout page will open in the user's browser, and the user will be prompted to enter the payment details and complete the process.
...
To make the integration seamless, when the checkout process ends within the browser's lightbox session, a "callback" function will be issued to your web page via JavaScript. Make sure to capture it to acknowledge that the session has finished.
...
The callback will have the `response` argument in which you will find the structure of a `MerchantNotificationRequest` with the following information:
...
You must listen to the message and validate the signature to ensure that the information comes from Placetopay. Then, you can use the `status` to update the information in your system or `requestId` to query the transaction status on your server.

Learn more about how to handle this message in [Notification](/en/checkout/notification).

<Note>
There were recently changes to the signing algorithm used in the notification. Learn more in [Migration to SHA-256](/en/checkout/migration-sha256).
</Note>

## Advanced Integration {{ id: 'advanced-integration' }}

Learn about the available methods and events in detail.
