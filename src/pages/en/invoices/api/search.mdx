...
This endpoint allows you to query the complete information of payment orders using the **debtor’s document**, the **reference**, or the **alternate reference**.
The response returns not only the requested payment order, but also all the orders associated with the linked document.

---

## Payment order search {{ id: 'search-invoices', tag: 'POST', label: '/cash/invoice/search' }}
...
<Row>
  ...
  <Col sticky>
    <br />
    <CodeGroup title="Payment order search" tag="POST" label="/cash/invoice/search">
       ```bash {{ title: 'Payment order search' }}
