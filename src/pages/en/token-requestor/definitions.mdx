...
Represent the authentication and authorization data linked to a consumer. These credentials allow secure access to the API and are designed to manage brand tokens only for the merchants associated with that consumer.

Additionally, the consumer can generate multiple credentials of their own, applying **include or exclude filters** that define more precisely which merchants can manage brand tokens with those credentials.
This provides more granular control over the operation and security of their merchants.
