...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Response">
      ...
      ```
      ```json {{ title: 'Failed Response, merchant invalid' }}
       {
         "status": {
           "status": "FAILED",
           "reason": "BR",
...
This operation allows modifying the attributes of an existing site in the administrative environment. It is designed to update **only** the fields included in the request, without altering other values of the site. It is available only for authenticated users with the appropriate site management permissions.

Each editable attribute must be included within the `data` object, along with the required `id` field that identifies the site to be updated. The operation is secure and controlled, preventing unauthorized changes to fields such as the merchant or consumer.
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/api/admin/sites/edit">
      ...
      ```
      ```bash {{ title: 'Partial field update' }}
         curl -X "POST" https://token-requestor.placetopay.ws/api/admin/sites/edit \
         -H "Content-Type: application/json" \
         -d '{
         "auth": {
      ...
      ```
      ```bash {{ title: 'Failed request: invalid ID format' }}
         curl -X "POST" https://token-requestor.placetopay.ws/api/admin/sites/edit \
         -H "Content-Type: application/json" \
         -d '{
         "auth": {
...
<Row>
  ...
  <Col sticky>
    ...
    ```json {{ title: 'Successful Response' }}
      ...
      ```
    ```json {{ title: 'Failed Request: Invalid data' }}
      {
        "status": {
          "status": "FAILED",
          "reason": "BR",
    ...
    ```
    ```json {{ title: 'Failed Request: Site not found' }}
      {
        "status": {
          "status": "FAILED",
          "reason": "BR",
    ...
    ```
    ```json {{ title: 'Failed Request: Invalid ID format' }}
      {
        "status": {
          "status": "FAILED",
          "reason": "BR",
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/api/admin/sites/index">
      ```bash {{ title: 'Request without filters (all accessible sites)' }}
      ...
      ```
      ```bash {{ title: 'Request with multiple filters' }}
      curl -X "POST" https://token-requestor.placetopay.ws/api/admin/sites/index \
      -H "Content-Type: application/json" \
      -d '{
        "auth": {
      ...
      ```
      ```bash {{ title: 'Failed request: Invalid UUID' }}
        curl -X "POST" https://token-requestor.placetopay.ws/api/admin/sites/index \
        -H "Content-Type: application/json" \
        -d '{
          "auth": {
...
<Row>
  ...
  <Col sticky>
    <br />
    <CodeGroup title="Response">
      ```json {{ title: 'Request without filters (all accessible sites)' }}
      {
        "status": {
          "status": "OK",
          "reason": "00",
      ...
      ```
      ```json {{ title: 'Request with multiple filters' }}
          {
              "status": {
                  "status": "OK",
                  "reason": "00",
      ...
      ```
      ```json {{ title: 'Failed request: Invalid UUID' }}
        {
            "status": {
                "status": "FAILED",
                "reason": "BR",
