export const sectionMode = 'nav';

import EnrollSequence from "@/components/react-flow/SequenceDiagram/Diagrams/TokenRequestor/en/EnrollSequence"


export const description = 'enroll-card.';

export const apiRefs = ['/api/tokens/enroll'];

# Enroll card
<Row>
  <Col>
    This API registers a card and converts it into a token so the consumer can use the token for subsequent payments.

    Its function is to register a card and convert it into a brand token so that the consumer can use the token for subsequent payments at various providers.
    Before consuming this API, a merchant registered with the brand must be on the platform.
  </Col>
  <Col>
    <EndpointsOverview />
  </Col>
</Row>

<EnrollSequence />

<ApiDownloads />

## Use of the Request Identifier - _**requestId**_

If the user loses connection or is unable to retrieve the generated token,
they can call the token generation service again. This service uses the request
identifier to recover the existing token without the need to generate a new one from the brand.

However, if the endpoint is called again with the same identifier
**after 5 minutes** of generating the token, it will return an error
indicating that the identifier already exists.

---


## Enroll Card {{ id: 'enroll-card', tag: 'POST', label: '/api/tokens/enroll' }}

<Row>
  <Col>
    <ApiReader
      path="/api/tokens/enroll"
      method="POST"
      api={props.refs}
    />
  </Col>
  <Col sticky>
    <br />
      <CodeGroup title="Solicitud" tag="POST" label="/api/tokens/enroll">
          ```json {{ 'title': 'Token for card' }}
          {
              "auth": {
                  "login":"aabbccdd1234567890aabbccdd123456",
                  "tranKey":"ABC123example456trankey+789abc012def3456ABC=",
                  "nonce":"enQ4dXh3YWhkMWM=",
                  "seed":"2023-06-21T09:56:06-05:00"
              },
              "siteId": "de394911-4219-4f1f-ae21-abf340e6e605",
              "requestId": "972293d1-5954-46e3-b00f-153f5aea03cf",
              "instrument": {
                  "card": {
                      "number": "4111111111111111",
                      "expiration": "12/23"
                      "cvv": "758"
                  }
              }
          }
          ```

          ```json {{ 'title': 'Token for token' }}
          {
              "auth": {
                  "login":"aabbccdd1234567890aabbccdd123456",
                  "tranKey":"ABC123example456trankey+789abc012def3456ABC=",
                  "nonce":"enQ4dXh3YWhkMWM=",
                  "seed":"2023-06-21T09:56:06-05:00"
              },
              "siteId": "de394911-4219-4f1f-ae21-abf340e6e605",
              "requestId": "972293d1-5954-46e3-b00f-153f5aea03cf",
              "instrument": {
                  "brandToken": {
                      "token": "4111111111111111",
                      "expiration": "12/23"
                  }
             }
          }
          ```
      </CodeGroup>
  </Col>
</Row>

<Row>
  <Col>
    <ApiReader
      path="/api/tokens/enroll"
      method="post"
      type="response"
      api={props.refs}
    />
  </Col>
  <Col sticky>
    <br />
    <CodeGroup title="Response">
    Responses with `FAILED` status can use HTTP `200` when the request was received correctly but the card was rejected by a validation or by the brand. Request validation errors use HTTP `400`, and authentication errors use HTTP `401`.

    ```json {{ title: '200' }}
    {
        "status": {
            "status": "OK",
            "reason": "00",
            "description": "The request has been successfully processed"
        },
        "data": {
            "brandToken": {
                "id": "a2e09e1d-e4a5-4b59-9e2f-cb9953c0acc7",
                "token": "5455950000007522",
                "expiration": "12/28",
                "requestId": "972293d1-5954-46e3-b00f-153f5aea03cf",
                "additional": {
                    "tokenRequestorID": "50160269427",
                    "artURL": "https://token-requestor.placetopay.dev/images/card-art/master.png",
                    "issuerName": "Test Issuer",
                    "lastDigits": "1234",
                    "appliedPrograms": ["VCES"]
                }
            }
        }
    }
    ```

    ```json {{ title: '200 - reason 05' }}
    {
        "status": {
            "status": "FAILED",
            "reason": "05",
            "message": "The request has been rejected",
            "date": "2026-09-17T01:10:36+00:00"
        }
    }
    ```

    ```json {{ title: '200 - reason 14' }}
    {
        "status": {
            "status": "FAILED",
            "reason": "14",
            "message": "A valid card was not provided",
            "date": "2026-09-17T01:11:19+00:00"
        }
    }
    ```

    ```json {{ title: '200 - reason X3' }}
    {
        "status": {
            "status": "FAILED",
            "reason": "X3",
            "message": "The request is invalid",
            "date": "2026-09-17T01:14:33+00:00"
        }
    }
    ```

    ```json {{ title: '400 - reason XN' }}
    {
        "status": {
            "status": "FAILED",
            "reason": "XN",
            "message": "A valid card has not been provided",
            "date": "2026-09-17T01:12:13+00:00"
        }
    }
    ```

    ```json {{ title: '400 - reason BR' }}
    {
        "status": {
            "status": "FAILED",
            "reason": "BR",
            "message": "The site id field must be a valid UUID.",
            "date": "2026-09-17T01:13:52+00:00"
        }
    }
    ```

    ```json {{ title: '401 - reason BR' }}
    {
        "status": {
            "status": "FAILED",
            "reason": "BR",
            "message": "The data provided is invalid",
            "date": "2026-09-17T01:13:31+00:00"
        }
    }
    ```
    </CodeGroup>
  </Col>
</Row>
