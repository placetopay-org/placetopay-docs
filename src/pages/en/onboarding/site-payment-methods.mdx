...
Los dos identificadores los publica el resultado de un proceso: `paymentMethodId` sale de `result.paymentMethods[].id` y el `id` de la habilitación, de `result.sites[].paymentMethods[].id`. [Consultar un comercio](/en/onboarding/show-merchant) también los devuelve: el `paymentMethodId` es el `customerPaymentId` de cada habilitación, y el `id`, su propio `id`.
