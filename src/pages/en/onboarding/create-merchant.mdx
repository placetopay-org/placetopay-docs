...
- **El sitio distingue mayúsculas donde la raíz no.** `languages` y `currencies` de la raíz aceptan cualquier grafía, pero `sites[].language` y `sites[].currency` exigen la del catálogo, que hoy escribe el idioma y la moneda en mayúscula (`ES`, `COP`). Un `"language": "es"` dentro de un sitio responde `422` — y el mensaje del error te dice la grafía que espera.
...
- [Consultar un comercio](/en/onboarding/show-merchant) — leer el comercio ya registrado
