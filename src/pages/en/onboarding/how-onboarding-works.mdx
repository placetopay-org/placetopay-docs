...
Tu servidor llama a [`POST /api/merchants`](/en/onboarding/create-merchant) con el comercio completo y el header `Idempotency-Key`.
...
Con el `processId` consultas [`GET /api/processes/{processId}`](/en/onboarding/process-status) hasta que el proceso llegue a un estado final.
...
El detalle de la firma y de cómo implementar el receptor está en [Notificación](/en/onboarding/notification).
...
- [Primeros pasos](/en/onboarding/quickstart) — un comercio mínimo, de principio a fin
- [Autenticación](/en/onboarding/authentication)
- [Idempotencia](/en/onboarding/idempotency)
