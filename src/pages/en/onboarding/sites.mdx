...
<Note type="warning">
  Una credencial generada se publica en `result.sites[]` del proceso. Guarda el resultado en cuanto lo recibas: después solo puede volver a leerse con [Consultar un comercio](/en/onboarding/show-merchant), que exige el permiso `onboarding:read`.
</Note>
...
<Note type="warning">
  `integration.notificationUrl` puede ser sobrescrito por una integración de sitio de tipo `notifier`. Ver [Integraciones del sitio](/en/onboarding/site-integrations).
</Note>
...
## Metadatos del sitio {{ id: 'site-metadata' }}
...
<Row>
  <Col>
    ...
    Es el único bloque de un sitio **sin vocabulario cerrado**: los nombres los eliges tú, no el catálogo.

    El valor de un nombre puede ser un **dato simple** —un texto, un número, un booleano— o una **estructura**: un objeto o una lista. Un dato simple conserva el tipo con el que lo envías: un número sigue siendo número y un booleano, booleano.
  </Col>
  <Col sticky>
    ```json {{ title: 'metadata' }}
    {
...
### Qué admite un nombre y qué admite un valor {{ id: 'metadata-shape' }}

Un **nombre** admite hasta 64 caracteres y distingue mayúsculas.

<Note type="warning">
  Un nombre del **primer nivel** no puede ser un número entero: `"3"` o `"42"` responden `422`, porque una vez decodificado el JSON nada los distingue de una posición. `"01"` sí es un nombre válido, y la restricción **no rige dentro de una estructura**: `{ "3": 0, "6": 1.5 }` es un valor perfectamente aceptable.
</Note>

Un **valor** es un dato simple o una estructura. El anidamiento llega hasta **tres niveles**, y cada dato simple admite 1024 caracteres, esté suelto o dentro de una estructura.

<Note>
  Un objeto cuyos nombres sean exactamente `0`, `1`, … `n-1` se almacena como **lista**: tras decodificar el JSON es indistinguible de una. No es algo que nadie escriba a propósito, pero conviene conocerlo antes de elegir los nombres de una estructura.
</Note>

### Se fusiona, y se borra nombrando la clave {{ id: 'metadata-merge' }}
...
<Row>
  <Col>
    ...
    La fusión ocurre **por nombre de primer nivel, y el valor se reemplaza entero**: reenviar un objeto no lo combina con el que estaba guardado. Para retirar una entrada de un objeto guardado, reenvía el objeto entero sin ella.
  </Col>
  <Col sticky>
    <CodeGroup title="Retirar una clave">
...
### El tamaño se mide sobre el documento resultante {{ id: 'metadata-size' }}
...
<Note type="warning">
  Hay un segundo tope, el del **elemento del sitio**, y se mide sobre algo distinto: sobre **lo que la petición envía** —del que las colecciones `integrations` y `paymentMethods` sí están exentas—, no sobre lo que quedaría guardado. Lo que consume ese presupuesto es, por tanto, el `metadata` que viaja en esa petición, no el que el sitio ya tenía: un documento que ha crecido no estrecha tus peticiones futuras. Cuando es el elemento el que se pasa, el `422` **señala el sitio entero, no el bloque**.
</Note>
...
Los límites exactos están en la [referencia de la API](/en/onboarding/api/reference/merchants#update-a-merchant), junto con la longitud máxima de un nombre y de un valor.

### Dónde se lee {{ id: 'metadata-reading' }}

<Note type="warning">
  `metadata` **no viaja en `result`**, y por tanto tampoco en la notificación ni en `GET /api/processes/{processId}`. El documento guardado, ya fusionado, solo se lee con [Consultar un comercio](/en/onboarding/show-merchant), en `sites[].metadata`.
</Note>

Un cuerpo que no envía `metadata` deja el `result` exactamente igual que antes de que este bloque existiera.

### Qué no poner aquí {{ id: 'metadata-not-for' }}

<Note type="warning">
  **No es sitio para un secreto ni para un dato personal.** El documento se guarda en claro, se copia entero al registro de auditoría del back office —cuya retención no controla esta API—, se conserva en el contenido del proceso, donde nada lo oculta, y lo devuelve la consulta del comercio a cualquier token con el permiso `onboarding:read`.
</Note>
...
- Una comisión heredada de un medio de pago del sitio pasa a `0.00`. Ver [Medios de pago del sitio](/en/onboarding/site-payment-methods).
...
Puedes volver a leerlas con [Consultar un comercio](/en/onboarding/show-merchant), que devuelve las credenciales de cada sitio en claro.

Si lo que necesitas es reemplazarlas —porque quedaron expuestas, por ejemplo—, **fija unas nuevas**. Envía un `tranKey` distinto en una actualización del sitio: al cambiar de valor se escribe, y viaja en el `result` de ese proceso.
...
- [Integraciones del sitio](/en/onboarding/site-integrations) — `sites[].integrations`
- [Medios de pago del sitio](/en/onboarding/site-payment-methods) — `sites[].paymentMethods`
