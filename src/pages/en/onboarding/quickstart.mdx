...
- Tu **token de acceso** y la **URL base** del ambiente donde vas a trabajar. Si aún no los tienes, consulta [Autenticación](/en/onboarding/authentication).
...
<Note>
  Las tres respuestas llegan con código HTTP **200**, incluida la que informa de un fallo: la consulta funcionó correctamente y te está diciendo qué pasó. Ver [Errores](/en/onboarding/errors).
</Note>

## 4. Guarda el identificador del comercio {{ id: 'step-merchant-id' }}

Cuando el proceso termina bien, `result.merchantId` contiene el identificador del comercio registrado. Guárdalo: es el que usarás para [actualizarlo](/en/onboarding/update-merchant) más adelante.
...
- [Datos del comercio](/en/onboarding/merchant-data) — todos los campos que puedes enviar
- [Sitios](/en/onboarding/sites) — registrar los sitios de venta del comercio
