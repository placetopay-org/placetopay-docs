...
También lo devuelve [Consultar un comercio](/en/onboarding/show-merchant), en `paymentMethods[].id`, para todos los medios de pago del comercio, incluidos los que no se registraron desde esta API.
...
A diferencia de las [integraciones](/en/onboarding/integrations), aquí **los nombres de las claves son libres**: no hay un catálogo que los declare.
...
Los elementos se ordenan antes de compararse con una petición anterior, de modo que el orden en que los envíes no afecta a la idempotencia. Omitir `financialEntity` y enviarlo como `null` cuentan como lo mismo, igual que omitir `id` o enviarlo nulo. Ver [Idempotencia](/en/onboarding/idempotency).
...
- [Medios de pago del sitio](/en/onboarding/site-payment-methods) — cómo un sitio habilita uno de estos
