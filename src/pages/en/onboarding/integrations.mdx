...
Una integración se direcciona **por su `type`**. Al escribir no hay identificadores numéricos: enviar `id` o `siteId` dentro de un elemento responde `422`, aunque [Consultar un comercio](/en/onboarding/show-merchant) publique un `id` por integración.
...
Cambiar el valor de una clave de `settings` sí produce un contenido distinto, y por tanto un `409` si reutilizas la clave. Ver [Idempotencia](/en/onboarding/idempotency).
...
- [Integraciones del sitio](/en/onboarding/site-integrations) — la colección equivalente dentro de un sitio
- [Referencia de la API](/en/onboarding/api/reference/merchants) — las claves que admite cada tipo de integración
