...
### Add extra fields to the custom export

At the end of the right-hand column, the user can dynamically add columns corresponding to the extra values of the transaction.

<CenteredContainer>
![addextra](/en/payments/newpanel/migration/transaction/addextra.webp) 
</CenteredContainer>

When the user types in a new extra field, it is automatically added to the selected fields column and included in the template without having to drag it manually.

<CenteredContainer>
![addcampo](/payments/newpanel/migration/transaction/addcampo.webp) 
</CenteredContainer>

These extra values are temporary while they remain on the left side (list of available values), but if they are on the right side, they are saved in the template and will be considered in future exports until they are deleted or the template is modified again.
