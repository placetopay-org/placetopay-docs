...
The **EBT** payment method is available and ready to use through [Webcheckout](/checkout). Placetopay securely and automatically manages the entire payment process.  
...
To process a [charge using a token](/checkout/api/reference/token#charge-using-token), the required fields are the same as in the session creation:  
