...
Once activated, the webhook will send a notification with the following structure when an ACH return is generated on your site:

```plaintext {{ title: 'Headers' }}
X-Signature: [signature to validate authenticity]
...
To ensure the authenticity of the webhook notification, each Placetopay webhook includes a signature in the `X-Signature` header. You can validate this signature by generating an HMAC SHA-256 hash using the notification body (`body`) and your site’s transaction key (`tranKey`).

```javascript {{ title: 'Validation in JavaScript' }}
const crypto = require('crypto');

// Request body and received signature
