...
| `filters (Optional)`  | filters | Structure that allows extending the search.                            |
...
### Example of a request with the “filters” option {{tag:'POST', label:'/invoice/filters/'}}
...
|-----------| ------------------------ |--------------------------------------------------------     |
| `auth`        | [Authentication](/payments/microsite-customized/authentication)| Authentication information.         |
| `data`        | data                      | Set of invoices to block or release.                            |
...
**Example of a failed response in a mass invoice blocking (reason: MH).**
