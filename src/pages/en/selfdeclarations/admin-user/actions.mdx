...
When logging in as the declaring user, they must download the electronic key to perform their duties within the company. This electronic key can only be downloaded once. If the user loses it, they can request revocation of the electronic key, which can only be done by an administrator. This will complete the user's association with a company.
...
- **Reissue Electronic Key:** If the declaring user loses the electronic key file and requests its revocation, this action will allow the declaring user to download the electronic key again.
