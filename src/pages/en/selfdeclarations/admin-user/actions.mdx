...
<Note>
   To complete the registration and access the system, the user must be linked to at least one declaration.
</Note>
...
The user must enter the registration information and create a password. The password will allow them to log in and will also be requested when performing actions on the company's declarations. It is clarified that the user will not be able to log in to the system until they are associated with at least one company and one declaration. To finish, the user presses **Complete Registration**.
