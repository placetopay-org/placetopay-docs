...
In this process, providing **additional data** is crucial to enhance user experience and ensure security. Here are some reasons to send extra information:

1. **Enhanced risk assessment:**
Supplying more details about the transaction and the customer allows issuers and authentication systems to perform a more accurate risk evaluation. Data such as purchase history, geographic location, and device information help issuers determine if risk-based authentication is possible, potentially avoiding extra steps like entering an OTP.

2. **Minimize user friction:**
Sharing information like phone number, email, previous authentication history, and shipping details can help authenticate the transaction without direct customer intervention. If the risk assessment deems the transaction low risk, the user may not need to actively authenticate, resulting in a smoother experience—known as a "frictionless flow."

3. **Reduce decline rates:**
The more relevant information included in the authentication request, the less likely the issuer is to decline the transaction due to insufficient data to assess the buyer's authenticity. This can improve transaction approval rates.

4. **Fraud prevention:**
Providing additional data, such as device geolocation, IP address, browser behavior, and previous authentication details, helps issuers more effectively identify fraudulent transactions.
...
To optimize the use of additional data, we invite you to review the [Use Cases](/three-d-s-server/api/integration/session) section, where you'll find clear examples of when and how to provide this information in your integrations. These cases detail specific scenarios where additional data is recommended or required to improve successful authentication rates.
...
- **Type:** `string`
- **Description:** Duration since the account was created on the merchant's website.
...
- **Type:** `string`
- **Description:** Date of the last change made to the cardholder's account on the merchant's website.
...
- **Type:** `string`
- **Description:** Time since the last change to the account on the merchant's website.
...
- **Type:** `string`
- **Description:** Date of the last password change or recovery.
...
- **Type:** `string`
- **Description:** Time since the last password change or recovery.
...
- **Type:** `string`
- **Description:** Time since the payment method was enrolled in the account on the merchant's website.
...
`threeDSRequestorPriorAuthenticationInfo` is an object containing information about how the 3DS Requestor authenticated the cardholder before or during the transaction.
...
- **Type:** `string`
- **Description:** Contains the ACS Transaction ID of the previous authenticated transaction.
...
- **Type:** `string`
- **Description:** Contains the DS Transaction ID of the previous authenticated transaction.
...
- **Type:** `string`
- **Description:** Indicates if the cardholder is placing an order for goods with future availability or a release date.
...
- **Type:** `string`
- **Description:** Indicates if the cardholder is reordering previously purchased merchandise.

**Accepted values:**
- `MERCHANDISE_AVAILABLE` `01` – First time
- `FUTURE_AVAILABILITY` `02` – Reordered

- **Length:** 2 characters

**Example value:** `'01'` or `'MERCHANDISE_AVAILABLE'`
...
`threeDSRequestorAuthenticationInfoProperties` is an object containing information about how the 3DS Requestor authenticated the cardholder before or during the transaction.
...
- **Type:** `string`
- **Description:** UTC date and time of the cardholder's authentication.
...
For more details on how to use this extension and improve authentication rates by leveraging version 2.3.1, see [Session transaction with Bridging Message Extension](/three-d-s-server/api/integration/session-b-m-e)
