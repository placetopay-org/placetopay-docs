...
**Autorización:** Hace referencia al método indispensable de autorización con nuestra API para realizar peticiones. Dicho método se basa en **Bearer Token**. Para más información, consulta [Authentication](/three-d-s-server/api/integration/authorization).
...
- **threeDSChallengeInd:** 03/CHALLENGE_REQUESTED_REQUESTOR_PREFERENCE (Se solicita desafío desde el 3DS Requestor) Indica que la preferencia del 3DS Requestor es que se realice el desafío **este campo es opcional pero recomendado incluir**.
...
* **03/CHALLENGE_REQUESTED_REQUESTOR_PREFERENCE:** el requestor tiene la preferencia de que se aplique desafío.
* **04/CHALLENGE_REQUESTED_MANDATE:** el desafío es obligatorio.
...
- **threeRIInd:** 01/RECURRING_TRANSACTION (Transacción recurrente)
