...
export const apiRefs = ['/api/v1/merchants/{merchantID}/branches', '/api/v2/merchants/{merchantID}/branches/{branchID}'];
...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="Request" tag="POST" label="/api/{version}/merchants/{merchantID}/branches">
    ```bash {{ title: 'V1', label: '/api/v1/merchants/{merchantID}/branches' }}
    ...
    ```
    ```bash {{ title: 'V2', label: '/api/v2/merchants/{merchantID}/branches' }}
...
<br />
## Delete branches to a merchant {{ id: 'delete-a-merchant', tag: 'DELETE', label: '/api/v2/merchants/{merchantID}/branches/{branchID}' }}

To delete a branch, it must have no associated transactions. If this condition is met, the branch, along with its subscriptions and associated tokens, will be deleted.

<Row>
  <Col>
      <ApiReader
        path="/api/v2/merchants/{merchantID}/branches/{branchID}"
        method="DELETE"
        api={props.refs}
      />
  </Col>
  <Col sticky>
    ...
    <br />
    <CodeGroup title="Request" tag="DELETE" label="/api/v2/merchants/{merchantID}/branches/{branchID}">
    ```bash {{ title: 'cURL' }}
    curl -X "POST" https://3dss-test.placetopay.com/api/v2/merchants/{merchantID}/branches/{branchID} \
      -H "Content-Type: application/json" \
      -H "Accept: application/json" \
      -H "Authorization: Bearer ACCESS_TOKEN" \
    ```
