...
export const apiRefs = ['/api/v1/merchants', '/api/v2/merchants/{merchantID}'];
...
<Row>
  ...
  <Col sticky>
    ...
    <br />
    <CodeGroup title="Request" tag="POST" label="/api/{version}/merchants">
    ```bash {{ title: 'V1', label: '/api/v1/merchants' }}
    ...
    ```bash {{ title: 'V2', label: '/api/v2/merchants' }}
...
<br />
## Delete a merchant {{ id: 'delete-a-merchant', tag: 'DELETE', label: '/api/v2/merchants/{merchantID}' }}

To perform the deletion, the merchant and its branches must not have any associated transactions. If this condition is met, the merchant, its branches, the merchant's and branches' subscriptions, and the tokens associated with both will be deleted.

<Row>
  <Col>
      <ApiReader
        path="/api/v2/merchants/{merchantID}"
        method="DELETE"
        api={props.refs}
      />
  </Col>
  <Col sticky>
    ...
    <br />
    <CodeGroup title="Request" tag="DELETE" label="/api/v2/merchants/{merchantID}">
    ```bash {{ title: 'cURL' }}
    curl -X "POST" https://3dss-test.placetopay.com/api/v2/merchants/{merchantID} \
      -H "Content-Type: application/json" \
      -H "Accept: application/json" \
      -H "Authorization: Bearer ACCESS_TOKEN" \
    ```
