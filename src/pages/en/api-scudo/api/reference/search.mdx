...
export const apiRefs = ['/api/v1/validations/search']
export const title = 'Validation Search'
export const description = 'Search for validations in SCUDO by reference and amount.'

<ApiDownloads />

## Validation Search {{ id: 'validation-transaction', tag: 'POST', label: '/api/v1/validations/search' }}

<Row>
    <Col>
        This service allows searching for transactions by reference, currency, and amount. It is typically used in case of communication loss when creating a validation.
        <ApiReader
            path="/api/v1/validations/search"
            method="POST"
            type="header"
            api={props.refs}
        />
    ...
    <Col sticky>
        <CodeGroup title="Request" tag="POST" label="/api/v1/validations/search">
            ```bash {{ title: 'cURL' }}
                curl --request POST \
                --url https://scudo-uat.placetopay.net/api/v1/validations/search \
...
<Row>
    ...
    <Col sticky>
        ...
        <CodeGroup title="Response">
            ...
            ```
            ```json {{ title: '402: Unprocessable Entity' }}
            {
                "data": null,
                "status": {
                    "code": 1202,
