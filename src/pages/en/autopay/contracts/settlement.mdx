...
## Settle/Confirm a payment {{ id: 'settlement', tag: 'POST', label: '{{baseURl}}/autopay/settlement' }}
<Note type="warning">
...
<Note type="warning">
    **Authentication (Basic Authentication):** This endpoint requires authentication via the `Authorization` header.
    The merchant must validate the credentials configured for AutoPay on each incoming request. Learn more at [**Contract authentication**](/autopay/contract-authentication).
</Note>

<Note type="warning">
    Settlement is performed only for transactions that have been successfully processed and are in **APPROVED** status.
</Note>
<Row>
    <Col>
        This endpoint is used to confirm a previously processed payment by sending the transaction details.
        <ApiReader
