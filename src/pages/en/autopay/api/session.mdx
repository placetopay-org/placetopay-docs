...
export const title = 'Create or update an AutoPay session'
export const description = 'Create or update an AutoPay session and obtain the URL to authorize the autopay.'
...
3. **Activation**: Once the process is completed, you will receive a notification via webhook indicating the creation or update of the autopay. Alternatively, you can check the status using the [session query](/en/autopay/api/query) endpoint to verify whether the autopay has already been updated.
...
<Note type="warning">
    ...
    For more information about the behavior, validations, and applicable contracts, refer to the official Checkout documentation at: [Create a Checkout session](/en/checkout/api/reference/session#create-a-session).
</Note>
