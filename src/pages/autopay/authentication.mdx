...
export const description = 'Cómo autenticar solicitudes en AutoPay usando login, nonce, seed y tranKey.'

<HeroPattern />

# Autenticación API

Para interactuar con la API de AutoPay debes autenticar tus peticiones, de esta forma identificamos y validamos la información para que tus operaciones sean seguras. La API utiliza *Web Services Security UsernameToken Profile 1.1*.
...
<Note type="warning">
    Estas credenciales son propias del sitio y deben ser tratadas de forma segura. No comparta sus credenciales en áreas de acceso público como Github, código del lado del cliente u otros lugares de fácil acceso para terceros.
</Note>
