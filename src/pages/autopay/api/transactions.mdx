export const apiRefs = ['/api/client/transactions']

## Transacciones de un AutoPago {{ id: 'transactions-autopay', tag: 'POST', label: '/api/client/transactions' }}
...
<Row>
    ...
    <Col sticky>
        <br />
        <CodeGroup title="Solicitud" tag="POST" label="/api/client/transactions">
            ```bash {{ title: 'General' }}
            curl -X "POST" {{baseURl}}/api/client/transactions \
