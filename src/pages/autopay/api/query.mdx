export const apiRefs = ['/api/client/query']

## Consultar un AutoPago {{ id: 'query-autopay', tag: 'POST', label: '/api/client/query' }}
...
<Row>
    ...
    <Col sticky>
        <br />
        <CodeGroup title="Solicitud" tag="POST" label="/api/client/query">
            ```bash {{ title: 'General' }}
            curl -X "POST" {{baseURl}}/api/client/query \
