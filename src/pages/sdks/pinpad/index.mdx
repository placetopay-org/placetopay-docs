...
export const description = 'Guía del SDK PinPad para capturar y encriptar PIN de forma segura con Placetopay.'
...
En el siguiente diagrama se describe de forma visual con más detalle el ciclo de vida de la integración:
...
Una vez tengas la instancia, puedes ejecutar el método `render` para mostrar el `input` para el PinPad. Este método recibe un selector CSS para el contenedor donde se renderizarán los componentes del PinPad y como segundo parámetro las posiciones ([obtenidas del Backend](#backend-connection)) de los botones de acción:

```javascript
pinpad.render('#your-pinpad-container', positions)
```

El pinpad renderizado contiene un input para el PIN, así podrás obtener el valor ingresado por el usuario al momento de enviar el formulario:
...
| Ambiente | URL |
| --- | --- |
| TEST | `https://pinpad-dev.placetopay.ws` |
