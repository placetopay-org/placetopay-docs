...
- **PIN:** número de dígitos del titular de la tarjeta de 4 a 12 dígitos de longitud, consulte **PIN** para obtener más información.
...
<PinpadFlow class="px-2 -my-5 bg-white" />
...
| `format` | `number` | `required` <br/> Formato de encriptación del PIN <br/> Posibles valores: `ISO-0`, `ISO-1`, `ISO-3` |
| `pin` | `string` | `required` <br/> PIN ingresado por el usuario |
| `pan` | `string` | `required` <br/> Número de la tarjeta del usuario |
