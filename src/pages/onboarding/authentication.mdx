...
<Properties>
  ...
  <Property name="Secreto de firma" type="string" isRequired={false}>
    Solo lo necesitas si vas a recibir notificaciones. Sirve para verificar que un webhook salió realmente de Onboarding. Ver [Notificación](/onboarding/notification).
  </Property>
</Properties>
...
| `onboarding:read` | `GET /api/merchants/{merchantId}` | [Consultar un comercio](/onboarding/show-merchant) |
...
Ninguna de las dos respuestas trae `processId`: son fallos de la llamada, no de un proceso. Esa distinción se explica en [Errores](/onboarding/errors).

## ¿Qué sigue? {{ id: 'authentication-next' }}

- [Idempotencia](/onboarding/idempotency) — el otro requisito de toda petición de escritura
- [Crear un comercio](/onboarding/create-merchant)
- [Referencia de la API](/onboarding/api/reference/user) — verificar el token
