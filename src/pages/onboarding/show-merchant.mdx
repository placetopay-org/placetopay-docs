...
Requiere un token con el permiso `onboarding:read`, el mismo que [consultar el proceso](/onboarding/process-status). No lleva cuerpo, no admite parámetros de consulta y no pagina: la respuesta trae todas las colecciones completas, así que su tamaño crece con los sitios y los medios de pago del comercio.
...
Es el `result.merchantId` que devolvió el proceso de creación, y el mismo que usa [`PATCH /api/merchants/{merchantId}`](/onboarding/update-merchant). Debe ser numérico: uno que no lo sea responde `404`.
...
<Row>
  <Col>
    La respuesta trae el objeto `status` y, al mismo nivel, los datos del comercio —`id`, `name`, `document`, `address`…— con los mismos nombres que en la escritura. Ver [Datos del comercio](/onboarding/merchant-data).
    ...
    El ejemplo de la derecha es un extracto. El ejemplo completo y el contrato campo a campo están en la [referencia de la API](/onboarding/api/reference/merchants#show-a-merchant).
  </Col>
  <Col sticky>
...
Por la misma razón, esta consulta es la forma de recuperar las credenciales de un sitio si perdiste el resultado del proceso que las generó. Ver [Sitios](/onboarding/sites#lost-credentials).
...
El representante legal se guarda como **un solo nombre completo** —por eso [nombre y apellido viajan en pareja](/onboarding/update-merchant#paired-fields) al escribir—, así que al leerlo `name` y `surname` se reconstruyen con una regla aproximada y pueden no coincidir con los que enviaste:
...
`paymentMethods[].id` es el identificador con el que un `PATCH` modifica cada medio de pago, así que esta consulta es también la forma de obtenerlo para uno que no creaste desde esta API. Ver [Medios de pago](/onboarding/payment-methods#where-the-id-comes-from).
...
- **Metadatos.** `metadata` es el documento tal como quedó después de las [fusiones](/onboarding/sites#metadata-merge), o `null` si el sitio no tiene.
...
Ver [La comisión por sitio](/onboarding/site-payment-methods#commission).
...
Para actualizar, envía **solo lo que cambia**, como se explica en [Actualizar un comercio](/onboarding/update-merchant#partial-update). Lo que esta consulta sí te da son los identificadores que ese cuerpo necesita: `paymentMethods[].id`, `sites[].id` y, dentro de un sitio, `paymentMethods[].id` o `customerPaymentId`.
...
En todas, `status.reason` trae el código HTTP como número. El catálogo completo está en [Errores](/onboarding/errors).
...
- [Sitios](/onboarding/sites) — las credenciales y los metadatos del sitio
- [Referencia de la API](/onboarding/api/reference/merchants#show-a-merchant) — el ejemplo completo y el contrato campo a campo
