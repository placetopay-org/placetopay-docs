...
<Note type="warning">
  Una credencial generada se publica en `result.sites[]` del proceso. Guarda el resultado en cuanto lo recibas: después solo puede volver a leerse con [Consultar un comercio](/onboarding/show-merchant), que exige el permiso `onboarding:read`.
</Note>
...
Los límites exactos están en la [referencia de la API](/onboarding/api/reference/merchants#update-a-merchant), junto con la longitud máxima de un nombre y de un valor.

### Dónde se lee {{ id: 'metadata-reading' }}

<Note type="warning">
  `metadata` **no viaja en `result`**, y por tanto tampoco en la notificación ni en `GET /api/processes/{processId}`. El documento guardado, ya fusionado, solo se lee con [Consultar un comercio](/onboarding/show-merchant), en `sites[].metadata`.
</Note>
...
Puedes volver a leerlas con [Consultar un comercio](/onboarding/show-merchant), que devuelve las credenciales de cada sitio en claro.
