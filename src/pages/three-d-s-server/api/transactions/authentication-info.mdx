export const description = 'Consulta el estado de autenticación de una transacción en 3DS Server con su transactionID.'

# Obtener información de una autenticación

Este servicio le permite consultar el estado de una transacción que pasó por el proceso de autenticación del tarjetahabiente con el 3DS Server.

## Datos del servicio
|          |                                             |
|----------|---------------------------------------------|
| Endpoint | **/api/v2x/transactions/`{transactionID}`** |
| Método   | GET                                         |

**Nota:** `{transactionID}`se debe tomar del dato transactionID en [Crear Sesión](/three-d-s-server/api/sessions)

---

## Posibles errores

| Código | Causa                                       |
|--------|---------------------------------------------|
| 403    | El token no está autorizado.                | 
| 404    | Transacción no existe                       | 
| 500    | El servidor ha encontrado un error interno. | 
