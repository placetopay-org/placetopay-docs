...
export const apiRefs = ['/api/v1/merchants', '/api/v1/merchants/{merchantCode}/{acquirerBIN}'];
...
## Consultar un comercio {{ id: 'show-a-merchant', tag: 'GET', label: '/api/v1/merchants/{merchantCode}/{acquirerBIN}' }}

<Row>
  <Col>
      <ApiReader
        path="/api/v1/merchants/{merchantCode}/{acquirerBIN}"
        method="GET"
        api={props.refs}
      />
  </Col>
  <Col sticky>
    ...
    <br />
    <CodeGroup title="Solicitud" tag="GET" label="/api/v1/merchants/{merchantCode}/{acquirerBIN}">
    ```bash {{ title: 'cURL' }}
    curl -X "GET" https://3dss-test.placetopay.com/api/v1/merchants/{merchantCode}/{acquirerBIN} \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer ACCESS_TOKEN" \
