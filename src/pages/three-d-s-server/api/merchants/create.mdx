...
export const apiRefs = ['/api/v1/merchants', '/api/v2/merchants/{merchant}'];
...
<br />
## Eliminar un comercio {{ id: 'delete-a-merchant', tag: 'DELETE', label: '/api/v2/merchants/{merchant}' }}
...
<Row>
  <Col>
      <ApiReader
        path="/api/v2/merchants/{merchant}"
        method="DELETE"
        api={props.refs}
      />
  </Col>
  <Col sticky>
    ...
    <br />
    <CodeGroup title="Solicitud" tag="DELETE" label="/api/v2/merchants/{merchant}">
    ```bash {{ title: 'cURL' }}
    curl -X "POST" https://3dss-test.placetopay.com/api/v2/merchants/{merchant} \
      -H "Content-Type: application/json" \
      -H "Accept: application/json" \
      -H "Authorization: Bearer ACCESS_TOKEN" \
    ```
