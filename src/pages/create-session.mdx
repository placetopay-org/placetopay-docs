...
Para crear una sesión debes consumir nuestra API según tus necesidades, conoce los detalles del API en [API - Crear Sesion](/api/session-create).

### `auth` - Authenticación

Para crear una sesión debes autenticarte con nuestra API, conoce los detalles en [Autenticación](/authentication)

### `locale` - Localización

Puedes elegir el idioma de la sesión haciendo uso de los parámetros del api, conoce los detalles en [Localización](/localization)

### `payer` - Usuario Pagador

Parámetro opcional

Se debe enviar el `payer` si quieres "forzar" el correo electrónico y datos personales con los que se hará el pago.

### `buyer` - Usuario Comprador

Parámetro opcional

Se envía el `buyer` cuando desde tu integración ya conoces al usuario que está comprando o adquiriendo el servicio

`buyer` y `payer` se diferencian conceptualmente porque el comprador `buyer` de un producto no necesariamente es el mismo pagador `payer`. Por ejemplo, `John` está comprando unos tickets de cine pero su padre `Jaime` es quien los pagará.

### `payment` - Pago

En esta estructura se envía la información relacionada con el pago como los montos, monedas y referencias.

### `subscription` - Suscripción

Estructura que se usa para el caso de uso de Suscripción, ver más en [Suscripción]()
...
### `skipResult` - Saltar resultado
### `noBuyerFill` - No completar el Comprador
### `type` - Tipo de sesión
...
Las sesiones parciales tambien tienen condicinoes adicionales como:
...
Se usa para describir la frecuencia y las condiciones con las cuales se harán cobros recurrentes.

<Row>
  <Col>
    <Properties>
      <Property name="periodicity" type="string">
        Periodicidad de la factura `D` Día | `M` Mes | `Y` Año
      </Property>
      <Property name="interval" type="int">
        Intervalo asociado a la periodicidad.
      </Property>
      <Property name="maxPeriods" type="int">
        Número máximo de periodo (-1 en caso de que no haya restricción.)
      </Property>
      <Property name="nextPayment" type="string" isRequired={false}>
        Fecha del próximo pago.
      </Property>
      <Property name="dueDate" type="string" isRequired={false}>
        Fecha para finalizar el pago.
      </Property>
      <Property name="notificationUrl" type="string" isRequired={false}>
...
El pago puede ser dividoe en diferentes convenios registrados, además permite que cada parte de la transacción sea procesada por diferentes proveedores.
...
Cuando se realiza una suscripción se obitne un token, con este token puedes hacer cobros al cliente.
