export const description = 'Sistema API de autodeclaraciones de rentas con integración a Placetopay para crear, consultar y gestionar declaraciones.'
...
Este método permite consultar la información de la declaración (firmas, estado e información de pago, estado de la declaración), la consulta a este método debe ser realizada con el requestId obtenido en el método income-request-aio. El sistema debe consultar la información de las declaraciones por un proceso de sonda que se debe ejecutar con una iteración de 1 hora, y sólo debe ser consultado en caso de que las declaraciones no tengan un estado de pago "aprobado", en caso de la que declaración no requiera pago (valor de pago = 0) no se debe realizar proceso de sonda, el estado de la declaración será "Presentado".

## 3. Integración método income-pdf:
Éste método permite obtener el pdf de la declaración, la solicitud se realiza con el requestId obtenido del método income-request-aio; el pdf obtenido será un string codificado en base64, por ende debe ser decodificado en la integración realizada por el ente municipal.
