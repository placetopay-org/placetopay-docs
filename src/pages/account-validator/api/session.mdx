...
export const title = 'Crear una Sesión'
export const description = 'En esta página, encontrarás información sobre la API Crear y consultar el estado de una Sesión.'
...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="Solicitud" tag="POST" label="/api/create-session">
    ```bash {{ title: 'cURL' }}
...
<Row>
  <Col>
   ...
   <Col sticky>
      <CodeGroup title="Solicitud" tag="POST" label="/api/check-session-status">
      ```bash {{ title: 'cURL' }}
