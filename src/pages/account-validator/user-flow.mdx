export const title = 'Flujo del usuario'
export const description = 'Flujo de verificación de cuentas personales y comerciales en Account Validator.'

## Flujo del usuario {{id: "user-flow"}}

Una vez el usuario sea redireccionado a la verificación de cuentas seguirá el siguiente proceso:

<img src="/Account_validator/account_verification2.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

Al pulsar confirmar el usuario verá la siguiente pantalla donde podrá seleccionar el tipo de cuenta. Si es una **cuenta personal** o una **cuenta comercial**:

<img src="/Account_validator/account_verification3.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }}/>

## Cuenta personal {{id: "personal-account"}}

Si se selecciona la cuenta personal, el usuario tendrá que seleccionar el banco de la cuenta que desea validar. Si el banco no está en la lista tendrá la posibilidad de escoger el campo “otro”.

<img src="/Account_validator/personal_account.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

- **Para Banco Popular**

Si la cuenta pertenece al banco popular, el usuario deberá ingresar el tipo y número de cuenta, además el número de ruta y últimos 4 dígitos del número de seguridad social.

<img src="/Account_validator/personal_account1.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

- **Para otros bancos**

Si la cuenta NO pertenece al banco popular, la verificación se realizará a través de un servicio externo (PLAID):

<img src="/Account_validator/personal_account2.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

Al seleccionar el banco, visualizara una ventana emergente y continuar con el proceso PLAID:

<img src="/Account_validator/plaid5.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

Una vez recopilada la información, el validador de cuentas verificará si la cuenta existe y su estado de verificación, dependiendo del banco y tipo de cuenta seleccionada, para finalmente informar el resultado:

<img src="/Account_validator/personal_account3.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

    <Note>
      **Reintentos:** Si la cuenta ingresada no logró ser verificada, el verificador permitirá reintentar el proceso, hasta 3 veces (configurables). Si luego de agotar los intentos la verificación se realiza la verificación será rechazada automáticamente.
    </Note>

## Cuenta Comercial {{id: "business-account"}}

Si se selecciona la cuenta comercial, el usuario tendrá que seleccionar el banco de la cuenta que desea validar además del número de cuenta y una confirmación del mismo.

<img src="/Account_validator/business_account.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

Una vez recopilada la información, el validador de cuentas verificará si la cuenta existe y su estado de verificación, dependiendo del banco y tipo de cuenta seleccionada, para finalmente informar el resultado:

<img src="/Account_validator/business_account1.png" width="280" style={{ width: '300px', display: 'block', margin: '0 auto' }} />

    <Note>
      **Reintentos:** Si la cuenta ingresada no logró ser verificada, el verificador permitirá reintentar el proceso, hasta 3 veces (configurables). Si luego de agotar los intentos la verificación se realiza la verificación será rechazada automáticamente.
    </Note>
