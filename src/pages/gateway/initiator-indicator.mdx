...
Para transacciones Mastercard, se envía un indicador a la red que identifica si la operación fue iniciada por el tarjetahabiente o por el comercio.

## Orden de prioridad

Placetopay determina el indicador en este orden:

1. **Valor del comercio:** si la solicitud de pago incluye un `additional.initiatorIndicator` válido, se utiliza sin modificaciones y tiene prioridad sobre la configuración del sitio.
2. **Configuración del sitio:** si la solicitud omite el indicador o envía `null`, se utiliza el valor configurado en el sitio, cuando exista.
3. **Valor automático:** si no envías un indicador ni tienes uno configurado en el sitio, Placetopay lo determina automáticamente.

## Configurar un valor fijo por sitio

Utiliza `data.integration.initiatorIndicator` en los endpoints de Core para [crear o actualizar sitios](/core/reference/site). El campo es opcional y admite cualquiera de los indicadores de la tabla siguiente. Los valores inválidos para el sitio se rechazan.

El siguiente fragmento configura un valor fijo al crear un sitio (`/core/site/create`) o actualizarlo (`/core/site/update-all`). Incluye los demás campos requeridos por el endpoint:
...
Al actualizar un sitio, omitir el campo conserva el valor actual. Envía `null` para quitar el valor fijo y volver al cálculo automático cuando el comercio no envíe el indicador:
...
Puedes consultar el valor configurado en `integration.initiatorIndicator` de la respuesta de búsqueda de sitios.

Por ejemplo, un sitio configurado con `MERCHANT_COF` utiliza `CARDHOLDER_COF` si la solicitud de pago envía explícitamente ese valor en `additional.initiatorIndicator`. Si lo omite, se utiliza el valor del sitio: `MERCHANT_COF`.
...
|      MERCHANT_WITH_INSTALLMENTS      | Una transacción en una serie de transacciones realizadas conforme a un acuerdo entre un titular de tarjeta y un comerciante, donde el titular de la tarjeta autoriza al comerciante a almacenar los datos de la cuenta del titular de la tarjeta para que el comerciante pueda iniciar una o más transacciones futuras por un monto conocido durante una duración especificada, basada en una sola compra.                                                      | Compra un televisor por 1,000 USD y paga en cuatro cuotas iguales de 250 USD cada una (la primera transacción el comercio envía CARDHOLDER_WITH_INSTALLMENTS  y las tres transacciones restantes envía MERCHANT_WITH_INSTALLMENTS). Para Colombia, debe ser enviado apenas en caso de financiamiento realizado por el propio comercio. |
