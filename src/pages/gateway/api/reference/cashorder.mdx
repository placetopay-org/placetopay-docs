...
<Row>
    <Col>
        Este servicio permite eliminar una orden de pago que se encuentre pendiente.
...
```
```json {{ title: '400 Error - Orden de pago no encontrada' }}
{
    "status": {
        "status": "FAILED",
        "reason": "BR",
...
```
```json {{ title: '400 Error - estado Orden de pago' }}
{
    "status": {
        "status": "FAILED",
        "reason": "BR",
        "message": "La factura debe estar en estado pendiente para ser eliminada",
        "date": "2025-07-07T16:43:40-05:00"
    }
}
```
