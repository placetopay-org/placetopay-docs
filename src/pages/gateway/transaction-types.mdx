...
Este paso reserva un monto específico en la tarjeta del cliente. 

Esto se hace enviando el `RequestBody` a [API Gateway -> Procesamiento de transacción](/gateway/api/reference/transaction#process-request).
...
* Se debe reutilizar `internalReference` y `authorization` del objeto `preAuthorization` retornado en el paso de **Check-in**.
