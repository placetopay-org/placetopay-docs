...
export const description = 'Búsqueda de una referencia de API de billetes de avión';
export const apiRefs = ['/ticket/search'];

# Buscar ticket

Este endpoint te permite encontrar un ticket por su referencia interna (`internalReference`).
...
<Row>
    ...
    <Col stricky>
        <CodeGroup title="Solicitud" tag="POST" label="/ticket/search">
            ```bash {{title: 'cURL'}}
            curl --request POST \
