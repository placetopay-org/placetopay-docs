...
2. **Reautorización (Ajuste opcional):** Se puede aumentar el monto retenido si cambia el valor del servicio. No se permiten reducciones.
3. **Checkout (Captura o Liberación):** La retención se captura (se cobra) o se libera. Finaliza la transacción.
...
- Las operaciones fallidas marcadas con un estado de rechazo (con el código de razón `VR`) no afectan los pasos previos y pueden reintentarse.

## Estructura de transacciones

Aunque no es directamente relevante para la integración, es útil para los comerciantes comprender cómo el sistema representa las transacciones de preautorización internamente:

- **Check-in**, **Reautorización** y **Checkout** son tipos de transacción.
- Las tres están asociadas a una transacción principal de tipo **Preautorización**, que actúa como un contenedor que resume todo el flujo.
- Esta transacción principal se crea automáticamente cuando se realiza un **Check-in**.
...
- Un **Checkout** exitoso concluye la transacción principal, marcándola como aprobada y asignando el monto final.

Esta estructura jerárquica permite a los comerciantes realizar un seguimiento completo del ciclo de vida de un pago preautorizado en una única referencia consolidada.
...
- La transacción permanece en estado **pendiente** hasta que se realice un **Checkout**.
- El nodo `preAuthorization` en la respuesta almacena datos críticos para las operaciones posteriores.

## Reautorización

**Reautorización** permite al comerciante ajustar (incrementar) el monto de la retención original antes de la captura final.
