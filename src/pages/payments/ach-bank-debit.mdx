...
Una vez activado, el webhook enviará una notificación con la siguiente estructura cuando se genere una devolución ACH en tu sitio:
...
Para garantizar la autenticidad de la notificación, cada webhook de Placetopay incluye una firma en el encabezado `X-Signature`. Puedes validar esta firma con una hash HMAC SHA-256 utilizando el contenido del cuerpo de la notificación (`body`) y la llave de transacción (`tranKey`) de tu sitio.
