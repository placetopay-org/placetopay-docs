...
export const description = 'Conoce el sistema de pagos ACH y el proceso de retorno de ACH (ACH Return) para tu negocio.';
...
## ¿Qué es un retorno ACH?

Un **Retorno ACH** (ACH Return) ocurre cuando una transferencia o débito automático no puede completarse exitosamente. Por ejemplo, en Puerto Rico, aunque una transacción ACH puede ser aprobada inicialmente, el procesamiento efectivo de los fondos puede tomar entre 1 y 2 días hábiles. Durante este tiempo, la transacción puede fallar por varias razones, generando así un retorno.

### ¿Por qué ocurren los retornos ACH?
...
Cada retorno viene con un código específico que explica la causa del fallo. Estos códigos ayudan a los comercios a entender rápidamente el motivo del retorno y a tomar las medidas necesarias.

## Cómo identificar y consultar retornos de ACH en Placetopay

En Placetopay, los retornos ACH se identifican y gestionan de manera fácil. A continuación, te explicamos cómo consultar y clasificar estos retornos.

### Acceso a retornos de ACH

Puedes visualizar el estado de cada transacción y sus retornos asociados en el panel administrativo de Placetopay. Además, puedes exportar esta información en un archivo descargable para realizar análisis externos.

Para integraciones automatizadas, Placetopay también ofrece una API que permite a los sistemas externos consultar los retornos ocurridos. Esto facilita la integración directa de la información de retornos en los sistemas de gestión del comercio, mejorando la eficiencia operativa.

### Códigos de retorno de ACH y su significado

Placetopay genera automáticamente códigos de retorno específicos que indican el motivo del fallo en cada transacción. Estos códigos permiten a los comercios clasificar rápidamente los problemas y tomar las acciones correspondientes.
...
## Guía para revisar y exportar retornos ACH

Para revisar tus retornos ACH en Placetopay, sigue estos pasos:
...
2. **Identifica la transacción principal**:
   ...
   - Sin embargo, el tipo de transacción se actualizará a “Reversada”, lo que indica que fue anulada por el retorno y no se debe contar como una transacción finalizada.
   - En el detalle de la transacción, encontrarás una sección de “Lista de Reembolsos”, donde se muestra el retorno asociado.
3. **Visualiza la transacción de retorno**:
   - Esta transacción incluye datos esenciales de la transacción original, como el monto, moneda, y número de cuenta.
   - Además, se agrega un código de retorno que especifica el motivo de la anulación (por ejemplo, R01: Fondos insuficientes).
   - En la sección “Detalle transaccional” se incluyen datos adicionales específicos del retorno, como el identificador de colección y el número de lote.

### Campos específicos en la transacción de retorno
...
| **processBatchId**          | Número de lote en el archivo NACHA que contiene el retorno, facilitando la gestión de múltiples transacciones y retornos en un solo archivo.                                                                                                             |
...
| **clientBankAccountNumber** | Número de cuenta del cliente en la institución financiera, necesario para la verificación de la cuenta original de débito.                                                                                                                               |

## Generación de reportes de retornos ACH

Desde el panel administrativo, puedes generar un reporte detallado de los retornos ACH:

1. Ve a la sección de “Transacciones” y selecciona “Exportar Transacciones”.
2. Escoge la opción “Exportación EBUS ACH Returns”, que genera un reporte específico de retornos ACH.
...
```txt {{ title: 'Ejemplo de un reporte de retorno' }}
...
Este reporte se puede importar en herramientas de análisis para gestionar y visualizar los retornos de manera más eficaz.
...
## Uso de la API para de retornos ACH

Placetopay proporciona una API que permite obtener reportes de transacciones, permitiendo la integración de la información de retornos en sistemas de gestión sin intervención manual.

### Generación de reporte vía API

Para generar el reporte, usa el endpoint de `Request transaction report` con el filtro `type: ["CHARGEBACK"]` para obtener las transacciones de devolución.
...
Una vez creado el reporte, podrás consultarlo en el endpoint de `Get transaction report`. Este reporte incluye transacciones diferenciadas con el campo `type`, señalando si la transacción fue un “CHARGEBACK”.
