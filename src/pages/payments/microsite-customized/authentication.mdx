...
Para el servicio de micrositio personalizado existen las siguientes autenticaciones:
...
Placetopay siempre enviará el siguiente objeto de autenticación por cada petición realizada.
...
- **nonce:** Valor arbitrario que identifica a una petición como única.
...
Placetopay usa el servicio de autenticación para obtener el token que posteriormente será enviado en cada petición.
La autenticación básica se construye concatenando el username y el password,
codificado en base64.

<Note> La petición para obtener el token se realiza via GET </Note>
...
Al momento de realizar la petición de autenticación, Placetopay espera la siguiente estructura como respuesta:
``` json
...
El campo **expires** representa el timestamp de expiración del token, es decir, la fecha y hora exactas en las que el token dejará de ser válido.<br />
El valor debe enviarse en formato Unix timestamp en segundos (10 dígitos).<br />
**Ejemplo**:
``` json
1767225600 // 2026-01-01 00:00:00 UTC
```
Se recomienda mantener el estándar de expiración de 1 hora desde la generación del token.<br />
