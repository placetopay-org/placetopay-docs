...
Es el método con el cual se listan las facturas relacionadas con los parámetros de consulta descritos a continuación:
...
| `filters (Opcional)`  | [filters](/payments/microsite-customized/services#filters) | Estructura que permite extender la búsqueda.                            |
...
| Nombre    | Tipo                     |  Descripción                                                  |
|-----------| ------------------------ |-------------------------------------------------------- |
| `status`        | [Autenticación](/payments/microsite-customized/authentication)| Describe el estado de esta petición.         |
| `data`   | String   | Arreglo de órdenes de pago u órdenes que coincidan con los parámetros de búsqueda proporcionados.                            |
| `options`  | String   | Objeto de opciones adicionales que permiten modificar el comportamiento en el flujo de pago.  |
| `messages`| [messages](/payments/microsite-customized/services#messages)   | Objeto con los mensajes que se deben mostrar en cada evento definido. |
...
### filters {{id:'filters', tag:'POST', label:'/invoice/filters/'}}
...
Estructura que permite definir mensajes personalizados en los eventos del flujo de datos y se muestran al usuario en sesión.
Estas opciones sólo se pueden incluir en la respuesta del servicio de consulta.

| Nombre                 | Tipo   | Descripción |
|------------------------|--------|-------------|
| `messages` | Object | Contiene los mensajes personalizados.<br /><pre>{`{"event":{"es":"message","en":"message"}}`}</pre> |
| `messages.postSearch` | Object | Mapa de mensajes por idioma <br /><pre>{`{"es":"message","en":"message"}`}</pre> |
| `messages.postSearch.es` | String | Mensaje correspondiente al idioma indicado <br /><pre>{`{"es":"message"}`}</pre> <br /> Los idiomas permitidos son <br /> - "es" (Español) <br /> - "en" (Inglés) <br /> - "fr" (Francés) <br /> - "it" (Italiano) <br /> - "pt" (Portugués) |
...
`HOLD` con el objetivo que Micrositios no permita realizar un pago de la misma factura mientras esta no sea liberada.
...
|-----------| ------------------------ |--------------------------------------------------------     |
| `auth`        | [Autenticación](/payments/microsite-customized/authentication)| Información de autenticación.         |
| `data`        | [data](/payments/microsite-customized/services#data)                      | Conjunto de facturas a bloquear o liberar.                            |
| `revoke`      | Bool                     | Determina si se debe revocar el hold o no false: La factura quedará bloqueada true: La factura será liberada.                     |
| `siteId`      | String                   | Credencial de identificación del sitio                     |
| `locale `     | [locale](/checkout/localization)     | Indica el idioma de la plataforma. Permite dar manejo a traducciones en los  [Mensajes de error.](/payments/microsite-customized/error-handling)                  |
...
| `franchise`   | String                     | Código de la franquicia utilizado.                     |
...
| `lastDigits`     | String                     | Últimos 4 dígitos de la tarjeta con la que se realizó el pago.                     |
| `provider`       | String                     | Se envía el string "provider".                     |
