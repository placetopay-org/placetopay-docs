...
Los modificadores de transacción son configuraciones a nivel de sitio que ajustan el flujo de una transacción según el tipo de modificador definido. Actualmente, solo está disponible la opción "Tarifa".

## Tipo tarifa
Este tipo de modificador de transacción aplica una tarifa (comisión) adicional al costo original de la transacción solicitada según las configuraciones definidas. Este modificador solo se aplica a medios de pago que soportan pagos tipo dispersión, ya que el cobro se efectúa mediante una [dispersión](/gateway/transaction-types#dispersion) al sitio (acuerdo) configurado en el modificador de transacción.
Las configuraciones de un modificador de transacción de este tipo son:
...
- **Porcentaje del Monto**: Porcentaje que se añadirá a la tarifa (comisón) calculada en base de la transacción original.
- **Aplica Impuesto**: Define si el IVA será aplicado al costo adicional generado por la transacción.

### ¿Cómo se integra?
El modificador de transacción tipo tarifa es soportado a través de nuestra API [Web Checkout](/checkout), el cual permite integrar diversos medios de pago y tipos de pago que son compatibles con este modificador.

### ¿Cómo se calcula la tarifa de un transacción?
Este cálculo se realiza considerando la moneda configurada en el sitio o sitios asociados al modificador de transacción tipo tarifa. Antes de realizar el cálculo, se validan las monedas de la petición y la del sitio configurado con el modificador. Por ejemplo, en caso de que la moneda del sitio sea `USD` y no coincida con la moneda de la petición `COP`, se llevará a cabo la conversión del **Monto Fijo** configurado de `USD` a `COP`, utilizando la tasa de conversión vigente al momento de la petición,

Una vez se validan las monedas, se desglosa la comisión calculada en función del monto original de la transacción en los [campos adicionales](/checkout/additional-fields) de la petición original enviada por el sitio. Estos campos incluyen **Subtotal**, **Costo de la transacción**, y, si el modificador de transacción tiene habilitada la opción  **Aplica Impuesto**, también se añadirá **IVA del costo de la transacción**. Estas descripciones no soportan cambio de idioma y se crean en base al idioma enviado en la primera petición del sitio.
...
* Monto Fijo:  0.35
* Porcentaje Monto: 5%
* Aplica Impuesto:  si  (10% se obtiene del impuesto configurado en el sitio)
