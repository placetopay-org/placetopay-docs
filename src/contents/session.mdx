...
<Row>
  <Col>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://checkout-test.placetopay.com/api/session \
...
## Consultar una sesión {{ tag: 'GET', label: '/api/session/:requestId' }}
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://checkout-test.placetopay.com/api/session/000000 \
      -H "Content-Type: application/json" \
      -d '{
          "auth": {
            "login": "c51ce410c124a10e0db5e4b97fc2af39",
