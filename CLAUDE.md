...
```bash
npm run dev                          # Start development server at localhost:3000 (runs generate:search-index + generate:api-downloads first)
npm run build                        # Production build (static export to /out); also runs both generators
npm run lint                         # ESLint
npm run generate:search-index        # Regenerate .cache/search/api-sections.json (API content for search)
npm run generate:api-downloads       # Regenerate public/downloads/ (bundled YAML/JSON specs + Postman collections)
...
`npm run dev` and `npm run build` depend on two generated artifacts:
- `.cache/search/api-sections.json` — extracted API request/response content merged into the search index by `src/mdx/search.mjs`
- `public/downloads/` — downloadable OpenAPI specs and Postman collections generated by `scripts/makeApiDownloads.js`

Both are regenerated automatically at the start of `dev`/`build`. Page URL/locale derivation for the search index lives in `src/mdx/pageInfo.mjs` and is shared between `scripts/makeSearchIndex.mjs` and `src/mdx/search.mjs` — keep both sides using it.
